...
> `scrapling` は `pip install "scrapling[all]"` でインストールすると PATH に入るランチャです。絶対パスで指定したい場合は `where scrapling` (Windows) / `which scrapling` (macOS/Linux) で確認した実行ファイルを `command` に書いてください。
